...
terraform {
  ...
  # backend "local" {
  #   path = "./backend/backend-terraform.tfstate"
  # }

  backend "s3" {
    bucket = "imazy-terraform-state-repo"
    region = "ap-south-1" 
    key = "backend/s3-backend-terraform.tfstate"
...
output "my_region" {
  value = var.my_region
  description = "region avlue"
}


output "my_cider_range" {
  value = var.my_cidr_range
...
data "aws_region" "available_region" {}
...
resource "aws_subnet" "my_public_subnet" {
  ...
  cidr_block              = cidrsubnet(var.my_cidr_range, 4 , each.value + 2)
  vpc_id                  = aws_vpc.my_vpc.id
  map_public_ip_on_launch = true
  availability_zone       = tolist(data.aws_availability_zones.my_available_zones.names)[0]
  tags = {
    Name      = "imazy-${each.key}"
    terraform = true
  }
...
resource "aws_subnet" "my_private_subnet" {
  cidr_block = cidrsubnet(var.my_cidr_range, 4 , 8)
  vpc_id     = aws_vpc.my_vpc.id
}
...
resource "aws_instance" "my_web-server" {
  ...
  tags = {
    ...
    Note = "this version_check for imazy after backend"
  }
}
